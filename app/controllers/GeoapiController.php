...
class GeoapiController extends BaseController {
	...
	public function DistribOrganiz()
	{
		...
		$pp_datos= DB::select("select sum(PP) as PP, SUM(BEN) as BEN, count([departamen]) as departamen, sum([municipio]) as municipio, sum([memorando]) as memorando, sum([territorio]) as territorio  from
(select sum(PP) as PP, SUM(BEN) as BEN, [departamen], sum([municipio]) as municipio, count([memorando]) as memorando, sum([territorio]) as territorio  from
(select sum(PP) as PP, SUM(BEN) as BEN, [departamen], count([municipio]) as municipio, [memorando], sum([territorio]) as territorio  from
(select sum(PP) as PP, SUM(BEN) as BEN, [departamen], [municipio], [memorando], count([territorio]) as territorio  from 
(SELECT count(*) as PP,sum(n_asociado) as BEN, [departamen], [municipio], [memorando], [territorio] FROM [DABASE].[sde].[PROYECTOS_PRODUCTIVOS] WHERE fuente='GeoODK' group by territorio, memorando, municipio, departamen) as base 
group by memorando, municipio, departamen) as base2
group by memorando, departamen) as base3
group by departamen) as base4");		
